...
const getCordinat=(x1:number,y1:number ,x2:number,y2:number):void=>{ //arrow function untuk menampung nilai yng akan di input

    try{ 
        //sebagai exeption jika terjadi kesalahan pada kode program dan akan menjalankan yang ada pada catch
        // if (typeof x1 !== 'number'||typeof y1 !== 'number'||typeof x2 !== 'number'||typeof y2 !== 'number') 
        // { throw "Input kordinat dalam angka"}
...
const sumDigit=(n:number):void=>{
    try{
        // if (typeof n !== 'number') { throw `${n} is not number , try again`}
        if (n >= 10000) { throw `${n} harus lebih kecil dari 10000`} // mengecek jika n lebih dari 10_000
    
        const num:any[] = n.toString().split('')//menkonvert n menjadi array dengan split agar bisa menggunkan function reduce 
...
const elapsedTime=(detik :number):void=>{

    try{
        // if (typeof detik !== 'number') { throw `${detik} is not number , try again`}
...
const getRandomPassword=(pass: number | string):void=>{
...
const covertToRupiah=(value:number, currencyType:string):void=>{
    const rupiah = (number:number)=>{
        ...
        if ( currencyType=='yen' ){
            ...
            console.log(rupiah(convertMoney).replace(',00',''))
...
covertToRupiah(1000,'yen')//Rp 130.120
// covertToRupiah(100,'usd')//Rp 1.438.250
// covertToRupiah(100,'euro')//Rp 1.600.000
// covertToRupiah(1000,'')//no match type currency'


/* ===================== NO 11 ===================== */

const getHavier=(w1 :number,w2:number,w3:number):void=>{
    try{
    // if (typeof w1 !== 'number'||typeof w2 !== 'number'||typeof w3 !== 'number') 
    // { throw "Input kordinat dalam angka"}
    let num:any[] = [w1,w2,w3] // menampung nilai w1,w2,w3 pada num[]
    console.log
...
const getDays=(month : number,year:number):void=>{
...
const isPalindrome=(angka:number):void=>{
   try {
    // if (typeof angka !== 'number'){ throw `${angka} is not an number`}
...
const getProsentase=(income1:number, income2:number):void=>{
    try{
...
const totalGaji=(gaji1:number,gaji2:number,gaji3:number):void=>{
try {
    // if(typeof gaji1 !== 'number' || typeof gaji2 !== 'number' || typeof gaji3 !== 'number'){

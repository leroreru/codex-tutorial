...
class taxi extends vehiclePurchase {
    ...
    GetTotalIncomeVehicle():any{
        let IncomePerTIpe = 0;
        let arrTaxi = this.vehicle.filter((vehicle)=>vehicle.vehicleType=='TAXI').map((obj, index) => Object.assign({}, obj, taxiObj[index])); 

            arrTaxi.forEach(value => {
                IncomePerTIpe = IncomePerTIpe + (value.order * value.orderPerKM) 
            })
            
            return `TOtal Income Taxi : ${this.numberFormat(IncomePerTIpe)}`

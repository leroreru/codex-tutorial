...
CREATE OR REPLACE FUNCTION notCursorPaginate(num_page INT, limit_page INT)
RETURNS TABLE(
		businessentityid 	INT,
		nationalidnumber 	VARCHAR,
		firstname		person.person.firstname%TYPE,
...
SELECT * FROM notCursorPaginate(1,20)
DROP FUNCTION notCursorPaginate(num_page INT, limit_page INT)

...
const EditUser = (props) => {
  ...
  // console.log('test', userx[0].id)

  const handleRegistration = async (data) => {
    dispatch(UpdateUser(data), props.closeModal())
    // if (data.status){
    //   if(data.result.status === 200){
    //     alert.succesAlert(`ID ${data.data.result}`, `${data.data.message}`)
    //   }
    //   else if (data.result.status === 400){
    //     alert.unSuccesAlert(`ID ${data.data.result}`, `${data.data.message}`)
    //   }
    //   props.closeModal()
                      ...
                      <div className="max-w-xl bg-white py-6 px-3 m-auto w-full mt-6">
                        <div className="grid grid-cols-1 gap-4 max-w-xl m-auto">
                          <div className="col-span-1">
                            <input
                              name="username" placeholder="Username" defaultValue={props.userById.username}
                          ...
                          <div className="col-span-1">
                            <input
                                type="text"
                              name="firstname" placeholder="Firstname" defaultValue={props.userById.firstname}
                              {...register(
                                "firstname",
                          ...
                          <div className="col-span-1">
                            <input
                              type="text"
                              name="lastname" placeholder="Lastname" defaultValue={props.userById.lastname}
                              {...register(
                                "lastname",
                          ...
                          <input
                              type="hidden"
                              name="id" defaultValue={props.userById.id}
                              {...register(
                                "id",
                                registerOptions.id
                              )}

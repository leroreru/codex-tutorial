...
import Content from './contentProd';
...
const Product = (props) => {
  ...
  let { product,message,refresh} = useSelector(state => state.productReducer)
  ...
  const prodID = (data)=>{
    navigate(`/product/update-product/${data.id}`,
    {
      state :{
        products : data,
      }
  ...
  return (
    <div>
      ...
      <ToastContainer/>
      <Content title="Product" isOpen={() => setIsOpen(true)}>
                          ...
                          <div className="px-1 py-1 ">
                            <Menu.Item>
                              {({ active }) => (
                                ...
                                onClick={()=>{prodID(data); setIsEdit(true)}}
                                  className={`${
                                    active
